...
from models_main.coupled_model import GLADS, SpecFO, Coupler_Hydro, Coupler_Flow_Hydro
...
hour = day/24
def get_dt(m):
    return max(15.0*hour, 50*hour + hour*(15.0-50)/(10-1e-14) * (m-1e-14))
...
for (msh_i,(msh_x,msh_y)) in enumerate(zip(x_cell.dat.data_ro, y_cell.dat.data_ro)):
    ...
    facet_function.dat.data[msh_i] = i_downstream[0][i_min]
mesh_c = df.RelabeledMesh(mesh, facet_functions, list(range(len(facet_functions))))
dx_c   = df.dx(domain=mesh_c)

df.VTKFile("catchments_hill.pvd").write(facet_function)
# print([df_moul.x[60], df_moul.y[60]])

# spatial field for k_s
k_s = df.Function(coupler.Q_cg)
# read random field
ds = xr.open_dataset("random_field_r5k_py.nc")
field = ds.field.values
x_rf = ds.x.values
y_rf = ds.y.values
interp = RegularGridInterpolator((x_rf, y_rf), field, bounds_error=False, fill_value=0.01)
values = interp(coords)
k_s.dat.data[:] = np.maximum(values, 1e-5)
df.VTKFile("k_s_test.pvd").write(k_s)
...
hydro.build_forms(m=0.05, dt0=dt0, e_v=1e-4, k_s=k_s, k_c=0.5, h_r=0.5, l_r=10, l_c=10, transition=True, omega=0.01, alpha_s=1.5, u_b=30, moulins=True)
